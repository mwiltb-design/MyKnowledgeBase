...
Subagents can also be bundled as part of Gemini CLI extensions by providing agent definition Markdown files (`.md`) to an `agents/` directory in your extension.
...
*   **cli_help:** An expert on Gemini CLI itself, ready to answer questions about features (i.e. “How do subagents work in Gemini CLI?”) by having direct access to the Gemini CLI documentation.
*   **codebase_investigator:** A specialized agent for exploring codebases, architectural mapping, bug root-cause analysis, and understanding system-wide dependencies.
